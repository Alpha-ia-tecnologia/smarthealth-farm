    ...
    <>
      ...
      {resumoQuery.isError ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <KpiCard label="Alertas ativos" value={resumoQuery.data ? fmtNum(resumoQuery.data.ativos) : ""} carregando={resumoQuery.isPending} icon={BellRing} accent="danger" hint="abertos + em tratamento" info="Total de avisos que ainda exigem atenção: os que ninguém começou a tratar somados aos que já estão em tratamento. Quanto maior o número, mais situações de risco aguardam uma providência." footer={resumoQuery.data ? <div className="flex justify-end"><BotaoAnaliseIa rotulo="Alertas ativos" onClick={() => setDialogOp("ativos")} /></div> : undefined} />
          <KpiCard label="Desabastecimento iminente" value={resumoQuery.data ? fmtNum(resumoQuery.data.desabastecimento) : ""} carregando={resumoQuery.isPending} icon={PackageX} accent="danger" info="Quantos insumos estão prestes a faltar no estoque, segundo a previsão de consumo. São os casos mais urgentes, porque a falta afeta diretamente o atendimento." footer={resumoQuery.data ? <div className="flex justify-end"><BotaoAnaliseIa rotulo="Desabastecimentos" onClick={() => setDialogOp("desabastecimento")} /></div> : undefined} />
          <KpiCard label="Risco de vencimento" value={resumoQuery.data ? fmtNum(resumoQuery.data.vencimento) : ""} carregando={resumoQuery.isPending} icon={CalendarClock} accent="warning" info="Quantos insumos têm lotes perto da data de validade. São itens que precisam ser usados ou remanejados a tempo para não serem descartados." footer={resumoQuery.data ? <div className="flex justify-end"><BotaoAnaliseIa rotulo="Vencimentos" onClick={() => setDialogOp("vencimento")} /></div> : undefined} />
          <KpiCard label="Tratados" value={resumoQuery.data ? fmtNum(resumoQuery.data.resolvidos) : ""} carregando={resumoQuery.isPending} icon={SlidersHorizontal} accent="success" hint="alertas resolvidos" info="Quantidade de avisos que já foram resolvidos. Serve para acompanhar o quanto a equipe vem dando conta dos riscos identificados." footer={resumoQuery.data ? <div className="flex justify-end"><BotaoAnaliseIa rotulo="Tratativas" onClick={() => setDialogOp("tratados")} /></div> : undefined} />
        </div>
      )}
